    ...
    storage = new CloudinaryStorage({
      ...
      params: {
        ...
        ...(allowedExtensions.length && {
          allowed_formats: allowedExtensions.map((m) => m.split("/")[1]),
        }),
      },
    });
  } else {
  ...
  return {
    ...
    array: (name, max) => (req, res, next) =>
      upload.array(name, max)(req, res, async (err) => {
        if (err) return next(err);
        if (req.files) {
          for (const file of req.files) await attachFileMeta(file, req);
        }
        next();
      }),
      ...
      upload.fields(fields)(req, res, async (err) => {
        ...
        if (req.files) {
          for (const key of Object.keys(req.files)) {
            for (const file of req.files[key]) await attachFileMeta(file, req);
          }
        }
        next();
      }),
