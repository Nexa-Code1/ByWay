...
export const getCartService = async (req, res) => {
  ...
  cart.courses = cart.courses.map((item) => {
    const course = item.course.toObject();
    ...
    const isFavourite = wishlistCourseIds.includes(course._id.toString());
